...
### 🌐 Running Blockchain Node on Different Port

By default, Hardhat runs on `http://127.0.0.1:8545`. If you need to run it on a different port (for example, to avoid conflicts with other services), follow these steps:

#### Option 1: Specify Port When Starting Hardhat Node

```bash
# Run Hardhat node on port 8546 instead of default 8545
npx hardhat node --port 8546

# Or run on any other port
npx hardhat node --port 9545
```

#### Option 2: Configure Hardhat Network Settings
...
module.exports = {
  networks: {
    localhost: {
      url: "http://127.0.0.1:8546", // Custom port
      chainId: 31337,
      accounts: {
        mnemonic: "test test test test test test test test test test test junk"
      }
    },
    // Add multiple local networks if needed
    local8547: {
...
```bash
# .env file - Update RPC URL to match your port
HARDHAT_NETWORK=localhost
RPC_URL=http://127.0.0.1:8546  # Updated port
CHAIN_ID=31337
PRIVATE_KEY=your_private_key_here
```
...
```javascript
// your-backend/config/blockchain.js
module.exports = {
  development: {
    rpcUrl: process.env.RPC_URL || "http://127.0.0.1:8546", // Updated port
    chainId: process.env.CHAIN_ID || 31337,
    gasLimit: 8000000,
    gasPrice: "20000000000" // 20 gwei
  },
  production: {
    rpcUrl: process.env.RPC_URL, // Use environment variable
...
#### Step 3: Initialize SolChain API with Custom Port
...
class EnergyService {
    ...
    async init() {
        // Option 1: Use custom port in config
        const config = new SolChainConfig({
            rpcUrl: this.customPort ? `http://127.0.0.1:${this.customPort}` : undefined,
            chainId: 31337
        });
        
        // Option 2: Or use environment variable
        // The config will automatically read from .env file
        
        const result = await config.createAPIInstance();
        if (result.success) {
            this.solchain = result.api;
            console.log(`✅ SolChain API initialized on port ${this.customPort || '8545'}`);
        } else {
            console.error('❌ Failed to initialize SolChain:', result.error);
        }
    }

    // Test connection to verify port is working
    async testConnection() {
        try {
            const tokenInfo = await this.solchain.getTokenInfo();
            if (tokenInfo.success) {
                console.log(`🔗 Successfully connected to blockchain`);
                console.log(`   Network: ${this.solchain.config.rpcUrl}`);
                console.log(`   Chain ID: ${this.solchain.config.chainId}`);
                return true;
            } else {
                console.error(`❌ Connection test failed: ${tokenInfo.error}`);
                return false;
            }
        } catch (error) {
...
// Custom port (8546)
const energyServiceCustom = new EnergyService(8546);

module.exports = EnergyService;
```

#### Step 4: Deploy Contracts to Custom Port

```bash
# Deploy to custom port network
npx hardhat run scripts/deploy.js --network localhost

# Or if you defined custom network in hardhat.config.js
npx hardhat run scripts/deploy.js --network local8547
```

#### Step 5: Update Frontend Configuration (if applicable)

```javascript
// frontend/config/blockchain.js
const BLOCKCHAIN_CONFIG = {
  development: {
    rpcUrl: 'http://127.0.0.1:8546', // Updated port
    chainId: 31337,
...
If you're running multiple instances or environments:

```bash
# Terminal 1: Development environment (port 8545)
npx hardhat node --port 8545

# Terminal 2: Testing environment (port 8546)  
npx hardhat node --port 8546

# Terminal 3: Staging environment (port 8547)
npx hardhat node --port 8547
```

```javascript
// services/MultiEnvironmentService.js
class MultiEnvironmentService {
    constructor() {
        this.environments = {
            development: new EnergyService(8545),
            testing: new EnergyService(8546),
            staging: new EnergyService(8547)
        };
    }

    getService(environment = 'development') {
        return this.environments[environment];
    }

    async testAllConnections() {
        for (const [env, service] of Object.entries(this.environments)) {
            console.log(`🧪 Testing ${env} environment...`);
            const connected = await service.testConnection();
            console.log(`   ${connected ? '✅' : '❌'} ${env}: ${connected ? 'Connected' : 'Failed'}`);
        }
    }
}

// Usage
const multiEnv = new MultiEnvironmentService();
await multiEnv.testAllConnections();

// Use specific environment
const devService = multiEnv.getService('development');
const testService = multiEnv.getService('testing');
```

#### Docker Setup with Custom Ports
...
services:
  hardhat-node:
    ...
    working_dir: /app
    ports:
      - "8546:8545" # Map container port 8545 to host port 8546
    volumes:
      - ./blockchain:/app
  ...
  backend:
    ...
    ports:
      - "3000:3000"
    environment:
      - RPC_URL=http://hardhat-node:8545 # Use container name
      - CHAIN_ID=31337
    depends_on:
      - hardhat-node
    networks:
...
class PortChecker {
    ...
    static async findAvailablePort(startPort = 8545, maxPort = 8600) {
    ...
    static async checkBlockchainConnection(rpcUrl) {
        try {
            const { ethers } = require('ethers');
...
// Usage example
async function setupOptimalPort() {
    console.log('🔍 Checking for available ports...');
    
    // Check if default port is available
    const defaultPortAvailable = !(await PortChecker.isPortOpen(8545));
    
    if (defaultPortAvailable) {
        console.log('✅ Port 8545 is available - using default');
        return 8545;
    } else {
        console.log('⚠️ Port 8545 is busy - finding alternative');
        const availablePort = await PortChecker.findAvailablePort(8546);
...
```bash
#!/bin/bash
# setup-custom-port.sh

PORT=${1:-8546}  # Default to 8546 if no port provided

echo "🚀 Setting up SolChain on port $PORT"

# Update .env file
sed -i "s|RPC_URL=.*|RPC_URL=http://127.0.0.1:$PORT|g" .env

# Start Hardhat node on custom port
echo "📡 Starting Hardhat node on port $PORT..."
...
node -e "
const EnergyService = require('./your-backend/services/EnergyService');
const service = new EnergyService($PORT);
setTimeout(async () => {
  const connected = await service.testConnection();
  console.log(connected ? '✅ Setup complete!' : '❌ Setup failed');
...
chmod +x setup-custom-port.sh
./setup-custom-port.sh 8546
```

**🎯 Key Points:**
- Always update your `.env` file when changing ports
- Test connections after port changes
- Use environment variables for flexible configuration
- Consider Docker networking for production deployments
- Check for port conflicts before starting services

### 🌍 Running Backend and Blockchain on Separate Nodes

For production deployments, you'll often run the blockchain node and backend API on separate machines/servers. Here's how to configure this distributed setup:
...
│ Port: 8545      │    │ Port: 3000      │    │ Port: 5432      │
└─────────────────┘    └─────────────────┘    └─────────────────┘
```

#### Step 1: Configure Blockchain Node (Server 1)

```bash
# On blockchain server (10.0.1.100)
cd blockchain

# Update hardhat.config.js for remote access
```

```javascript
// hardhat.config.js (on blockchain server)
module.exports = {
  solidity: "0.8.19",
  networks: {
    localhost: {
      url: "http://0.0.0.0:8545", // Bind to all interfaces
      chainId: 31337,
      accounts: {
        mnemonic: "test test test test test test test test test test test junk"
      }
...
```bash
# Start blockchain node accepting remote connections
...
#### Step 2: Configure Backend API (Server 2)

```bash
# On backend server (10.0.1.101)
# Copy blockchain folder (just the API and contracts)
rsync -av blockchain-server:/path/to/blockchain/ ./blockchain/

# Update environment variables
```

```bash
# .env file on backend server
NODE_ENV=production
HARDHAT_NETWORK=production
RPC_URL=http://10.0.1.100:8545  # Blockchain server IP
CHAIN_ID=31337
PRIVATE_KEY=your_private_key_here

# Database connection (separate database server)
DATABASE_URL=postgresql://user:pass@10.0.1.102:5432/solchain
...
#### Step 3: Update Backend Configuration for Remote Blockchain

```javascript
// config/blockchain.js (on backend server)
module.exports = {
  development: {
  ...
  production: {
    rpcUrl: process.env.RPC_URL || "http://10.0.1.100:8545", // Remote blockchain
    chainId: parseInt(process.env.CHAIN_ID) || 31337,
    gasLimit: 8000000,
    gasPrice: "20000000000",
    timeout: 60000, // 60 seconds for remote calls
...
#### Step 4: Enhanced EnergyService for Remote Blockchain

```javascript
// services/EnergyService.js (enhanced for remote connections)
const { SolChainConfig } = require('../blockchain/src/config');
...
class EnergyService {
    ...
    async init() {
        try {
            ...
            if (result.success) {
                ...
                console.log(`✅ SolChain API connected to remote blockchain`);
            ...
            if (this.connectionRetries < this.maxRetries) {
                ...
                setTimeout(() => this.init(), 5000 * this.connectionRetries); // Exponential backoff
            } else {
                console.error(`💥 Max retries reached. Blockchain service unavailable.`);
                throw error;
            }
        }
    }
    ...
    async testConnection() {
        try {
            ...
            if (tokenInfo.success) {
                console.log(`🏥 Blockchain health check: OK (${responseTime}ms)`);
                console.log(`   Token: ${tokenInfo.data.name} (${tokenInfo.data.symbol})`);
                console.log(`   Supply: ${tokenInfo.data.totalSupply}`);
                return true;
            } else {
                console.error(`❌ Health check failed: ${tokenInfo.error}`);
                return false;
            }
        } catch (error) {
    ...
    // Periodic health check for remote blockchain
    startHealthCheck() {
        setInterval(async () => {
            try {
                ...
                if (!healthy) {
                    console.warn(`⚠️ Blockchain connection unhealthy - attempting reconnection`);
                    await this.init();
                }
            } catch (error) {
    ...
    // Wrapper method with connection validation
    async executeWithValidation(operation) {
        if (!this.isHealthy) {
        ...
        try {
            ...
            // If network error, mark as unhealthy and retry
            if (error.code === 'NETWORK_ERROR' || error.code === 'TIMEOUT') {
                this.isHealthy = false;
                console.warn(`Network error detected - will retry connection`);
            }
            throw error;
        }
    }
    ...
    async recordEnergyProduction(userId, kwhProduced) {
        return await this.executeWithValidation(async () => {
            return await this.solchain.mintTokens(userId, kwhProduced.toString());
        });
    }
...
```nginx
# nginx.conf (if using load balancer)
upstream blockchain_nodes {
    server 10.0.1.100:8545 weight=3;
    server 10.0.1.103:8545 weight=2; # Backup blockchain node
}

upstream backend_apis {
    server 10.0.1.101:3000;
    server 10.0.1.104:3000; # Load balanced backend
}
...
server {
    ...
    # Direct blockchain access (if needed)
    location /rpc/ {
...
#### Step 6: Docker Compose for Distributed Setup
...
services:
  # Blockchain Node (can be on separate server)
  blockchain-node:
    image: node:18-alpine
    working_dir: /app
    ports:
  ...
  # Backend API (can be on separate server)
  backend-api:
    build: 
      context: .
    ...
    environment:
      - NODE_ENV=production
      - RPC_URL=http://blockchain-node:8545  # Use service name for internal network
      - DATABASE_URL=postgresql://user:pass@database:5432/solchain
...
echo "🚀 Deploying SolChain in distributed mode..."

# Set environment
export NODE_ENV=production

# Deploy blockchain node (Server 1)
echo "📡 Deploying blockchain node..."
ssh blockchain-server << 'EOF'
cd /opt/solchain/blockchain
docker-compose -f docker-compose.blockchain.yml up -d
sleep 10
npm run deploy:production
EOF

# Deploy backend API (Server 2)
...
# Deploy database (Server 3)
echo "🗄️ Deploying database..."
ssh database-server << 'EOF'
cd /opt/solchain
docker-compose -f docker-compose.database.yml up -d
sleep 15
# Run migrations
docker exec solchain_backend npm run migrate
EOF
...
# Health checks
curl -f http://blockchain-server:8545 && echo "✅ Blockchain: OK" || echo "❌ Blockchain: FAIL"
curl -f http://backend-server:3000/health && echo "✅ Backend: OK" || echo "❌ Backend: FAIL"
curl -f http://database-server:5432 && echo "✅ Database: OK" || echo "❌ Database: FAIL"
```

#### Step 8: Monitoring and Alerts

```javascript
// monitoring/BlockchainMonitor.js
class BlockchainMonitor {
    ...
    async checkHealth() {
        ...
        try {
            const isHealthy = await this.energyService.testConnection();
            this.metrics.responseTime = Date.now() - startTime;
            this.metrics.connectionStatus = isHealthy ? 'healthy' : 'unhealthy';
    ...
    async sendAlertsIfNeeded() {
        ...
        if (this.metrics.connectionStatus !== 'healthy' && this.metrics.errorCount > 5) {
            await this.sendAlert('CRITICAL', 'Blockchain service is down');
        }
        
        // Alert if response time is too high
        if (this.metrics.responseTime > 10000) { // 10 seconds
            await this.sendAlert('WARNING', `High response time: ${this.metrics.responseTime}ms`);
        }
    }
    ...
    async sendAlert(level, message) {
        ...
        try {
            await fetch(process.env.SLACK_WEBHOOK_URL, {
                ...
                body: JSON.stringify({
                    text: `🚨 SolChain Alert [${level}]: ${message}`,
                    channel: '#alerts'
                })
            });
        } catch (error) {
...
#### Key Benefits of Distributed Setup:

1. **🔧 Scalability**: Scale blockchain and backend independently
2. **🛡️ Security**: Isolate blockchain node from public internet
3. **📈 Performance**: Dedicated resources for each service
4. **🔄 Redundancy**: Multiple nodes for high availability
5. **🧪 Environment Separation**: Dev/staging/prod environments
6. **💰 Cost Optimization**: Different server sizes for different services

#### Common Issues and Solutions:

**Connection Timeouts**:
```javascript
// Increase timeout values for remote connections
const config = new SolChainConfig({
...
```bash
# Open required ports
sudo ufw allow 8545/tcp  # Blockchain RPC
